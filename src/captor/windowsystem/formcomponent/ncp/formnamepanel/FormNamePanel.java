package captor.windowsystem.formcomponent.ncp.formnamepanel;
 ...
 *
 */
public class FormNamePanel extends FormComponent implements KeyListener {
    ...
    public FormNamePanel(Model model, DefaultMutableTreeNode node) {
        super(model, node);
        id = "";
        create();
    }
        ...
        for ( int i = 0; i < parent.getChildCount(); i++ )  {
            ...
            if ( obj instanceof FormPath )  {
                ...
                for ( int j = 0; j < v.size(); j++ )  {
                    ...
                    if ( fe.getId().equals("pt_name") )  {
                        String vname2 = fe.getValues();
                        if ( vname2.trim().equals(textField.getText().trim()) )  {
                            cleanErrorLine();
                            addErrorLine("This name is been used by another form.");
